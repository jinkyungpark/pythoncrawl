...
class GspiderSpider(CrawlSpider):
    ...
    def parse_start_url(self, response):
        yield scrapy.Request(url=response.url, callback=self.parse_item, meta={"main_cate_name": "ALL", "sub_cate_name": "ALL"})

    # ALL을 제외한 카테고리가 호출하는 부분 - 각 카테고리별 best100 추출 호출 및 2차 카테고리 추출
    def parse_sub_category(self, response):
        print("exclude ALL", response.url)

        # 1차 카테고리 이름 추출하기
        main_cate_name = response.css(
            "div.gbest-cate ul.by-group li.on a::text").get()

        # 1차 카테고리 best100 상품 출력하기 호출
        yield scrapy.Request(url=response.url, callback=self.parse_item, meta={"main_cate_name": main_cate_name, "sub_cate_name": "ALL"}, dont_filter=True)

        # 2차 카테고리 및 카테고리 명 추출 - 관련 상품군은 제외
        sub_category_addr = response.css(
            "div.navi ul li[class!='related'] a::attr('href')").getall()

        # 서브 카테고리 추출 - 관련상품군은 제외
        sub_cate_name = response.css(
            "div.navi ul li[class!='related'] a::text").getall()

        for idx, sub_addr in enumerate(sub_category_addr, 0):
            sub_url = response.urljoin(sub_addr)
            sub_name = sub_cate_name[idx]
            # print("2차 카테고리 url", sub_url, sub_name)
            yield scrapy.Request(url=sub_url, callback=self.parse_item, meta={"main_cate_name": main_cate_name, "sub_cate_name": sub_name}, dont_filter=True)
